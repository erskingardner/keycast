...
| Delegated NIP-98 can administer its own key | Private write kind 27236 and instance-bound read kind 27237, both always denied through NIP-46; same identity approved from its external store; bound instance/revision/body/nonce/reply key | Delegated-read/write denial, cross-instance/status-role checks, replay and context ambiguity tests, Unicode browser-client signing/encryption tests |
...
| Resource growth and starvation | Global/client/grant admission, RAII capacity release, fair recovery/outbox selection, per-grant/client/team storage quotas with reserved reply space, persistent byte/row/page caps, incremental retention, redacted status and monitor | Rotating-client noisy-grant test, budget saturation and SQLite-full recovery, bounded map tests, host-monitor tests and measured WAL/backlog after soak |
| One malformed grant or worker stops service | Per-grant validation/quarantine, incremental relay reload, tracked request/publisher/replication tasks, isolated per-request panics, transient database retries, bounded shutdown | Healthy grant continues beside corrupt envelope; changing relays preserves session; cancellation capacity tests |
